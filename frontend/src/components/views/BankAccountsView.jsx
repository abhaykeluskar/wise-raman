  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300 pb-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className={`p-6 rounded-3xl flex flex-col justify-between border-0 transition-all ${style('neu-flat-dark', 'neu-flat-light')}`}>
          ...
          <p className="text-xs text-slate-400 mt-2">Across {assetAccounts.length} savings/current accounts</p>
        </div>
        ...
        <div className={`p-6 rounded-3xl flex flex-col justify-between border-0 transition-all ${style('neu-flat-dark', 'neu-flat-light')}`}>
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-rose-400">
            {formatCurrency(totalLoans)}
          </h2>
          <p className="text-xs text-slate-400 mt-2">Across {loanAccounts.length} active loan accounts</p>
        </div>
      </div>

      {/* Asset Accounts List */}
      <h3 className={`text-lg font-black mt-4 ${style('text-[#F4F7FA]', 'text-[#17202A]')}`}>Savings & Current Accounts</h3>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {assetAccounts.length === 0 && (
          <p className="text-slate-400 text-sm italic">No asset accounts found.</p>
        )}
        {assetAccounts.map(acc => (
          <div key={acc.id} className={`p-5 rounded-3xl flex items-center justify-between border-0 transition-all ${style('neu-flat-dark', 'neu-flat-light')}`}>
            <div className="flex flex-col gap-1">
              <div className="flex items-center gap-2">
                <Landmark className="h-4 w-4 text-slate-400" />
                <span className="font-bold text-sm">{getBankName(acc.bank_id)}</span>
                <Badge variant="brand">{acc.subtype}</Badge>
              </div>
      ...
      {/* Loan Accounts List */}
      <h3 className={`text-lg font-black mt-4 ${style('text-[#F4F7FA]', 'text-[#17202A]')}`}>Loan Accounts</h3>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {loanAccounts.length === 0 && (
          <p className="text-slate-400 text-sm italic">No active loan accounts found.</p>
        )}
        {loanAccounts.map(acc => (
          <div key={acc.id} className={`p-5 rounded-3xl flex flex-col gap-3 border-0 transition-all ${style('neu-flat-dark', 'neu-flat-light')}`}>
            ...
            <div className="flex justify-between items-center bg-black/15 rounded-2xl p-3 gap-3">
              <div className="flex flex-col min-w-0">
                <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Loan Plan</span>
                <span className={`text-xs font-medium truncate ${style('text-[#F4F7FA]', 'text-[#17202A]')}`}>{acc.name}</span>
              </div>
              <div className="flex flex-col text-right shrink-0">
                <span className="text-[10px] uppercase tracking-wider text-slate-400 font-bold">Current EMI</span>
                <span className={`text-sm font-bold ${style('text-[#F4F7FA]', 'text-[#17202A]')}`}>{formatCurrency(acc.monthly_cap || 0)}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      {archiveAccounts.length > 0 && (
        <>
          <h3 className={`text-lg font-black mt-4 ${style('text-[#F4F7FA]', 'text-[#17202A]')}`}>Historical Archive & Legacy Data</h3>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            {archiveAccounts.map(acc => (
              <div key={acc.id} className={`p-5 rounded-3xl flex items-center justify-between border-0 transition-all ${style('neu-flat-dark', 'neu-flat-light')} opacity-75`}>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2">
                    <Landmark className="h-4 w-4 text-slate-400" />
                    <span className="font-bold text-sm">{getBankName(acc.bank_id)}</span>
                ...
                <div className="text-right">
                  <span className="text-xs text-slate-400 block">Stored Records</span>
                  <span className="text-xs font-semibold text-slate-500">Isolated from Active Spends</span>
                </div>
              </div>
            ))}
          </div>
