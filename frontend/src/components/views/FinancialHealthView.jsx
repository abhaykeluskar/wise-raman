...
export const FinancialHealthView = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    Promise.all([
      fetch(`${API_BASE_URL}/api/health-score`, { headers: fetchHeaders }).then(r => r.ok ? r.json() : null),
      fetch(`${API_BASE_URL}/api/analytics/anomalies`, { headers: fetchHeaders }).then(r => r.ok ? r.json() : []),
      fetch(`${API_BASE_URL}/api/analytics/financial-calendar`, { headers: fetchHeaders }).then(r => r.ok ? r.json() : null),
      fetch(`${API_BASE_URL}/api/analytics/lifestyle-inflation`, { headers: fetchHeaders }).then(r => r.ok ? r.json() : null)
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-300 pb-16">
      ...
      {/* 1. HEALTH SCORE TAB */}
      {activeTab === 'health_score' && healthScoreData && (
        <div className="flex flex-col gap-6">
          {healthScoreData.insufficient_data ? (
            <div className={`p-8 rounded-3xl border-0 text-center flex flex-col items-center justify-center gap-3 ${style('neu-flat-dark', 'neu-flat-light')}`}>
              ...
              <h3 className={`text-lg font-bold ${style('text-white', 'text-slate-800')}`}>{healthScoreData.display_title}</h3>
              <p className="text-xs text-slate-400 max-w-md">{healthScoreData.message}</p>
            </div>
          ) : (
            <>
              {/* Score Hero Card */}
              <div className={`p-6 sm:p-8 rounded-3xl border-0 flex flex-col md:flex-row items-center justify-between gap-6 ${style('neu-flat-dark', 'neu-flat-light')}`}>
                <div className="flex items-center gap-6">
                  <div className="relative flex items-center justify-center h-28 w-28 rounded-full border-4 border-emerald-500/40 bg-emerald-500/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {anomalies.map((anom, idx) => (
                <div key={idx} className={`p-5 rounded-3xl border-0 flex flex-col justify-between gap-4 ${style('neu-flat-dark', 'neu-flat-light')}`}>
                  <div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-slate-400">{anom.merchant}</span>
                      <span 
      ...
      {activeTab === 'lifestyle' && lifestyleData && (
        <div className="flex flex-col gap-6">
